 ...
 *
 * 每个工具由两部分组成：
 *  1. spec：JSON Schema 形态的工具定义（要发给 LLM）
 *  2. handler：实际执行逻辑（LLM 决定调用后由我们的代码运行）
 *
 * 设计要点：
 * - handler 永远返回 string，方便作为 role:'tool' 的 content 回灌给模型
 * - 任何错误也要 catch 后转成字符串，不能抛出去打断 Agent Loop
 *   （让模型自己看到错误信息、决定是否重试或换工具）
 */
import * as fs from 'fs';
import * as path from 'path';
import { retrieve, getKnowledgeDir } from './rag';

/** OpenAI 协议里 tool 的 spec 结构 */
export interface ToolSpec {
  type: 'function';
  function: {
    name: string;
    description: string;
    /** JSON Schema。OpenAI SDK 类型要求是 Record<string, unknown> */
    parameters: Record<string, unknown>;
  };
}

/** 工具实现统一签名：拿到 LLM 给的 JSON 参数 → 返回字符串结果 */
  ...
  spec: {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          filename: { type: 'string', description: '文件名（含扩展名），例如 "登录-最佳实践.md"' },
        },
        required: ['filename'],
      },
    },
...
const save_optimized_doc: { spec: ToolSpec; handler: ToolHandler } = {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          filename: {
            type: 'string',
            description: '保存的文件名，可选，默认 optimized-by-agent.md',
          },
        },
        required: ['markdown'],
      },
  ...
  handler: async (args) => {
    ...
    const out = path.resolve(process.cwd(), safeName);
    fs.writeFileSync(out, md, 'utf-8');
    return `✅ 文档已保存：${out}（${md.length} 字符）`;
  },
};

// ============ 工具 5：get_current_time ============
...
// ============ 导出注册表 ============
export const TOOLS: Record<string, { spec: ToolSpec; 
    handler: ToolHandler }> = {
  search_knowledge,
  list_knowledge,
  read_knowledge_file,
